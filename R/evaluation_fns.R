...
get_bias <- function(results, truth, n){
  ...
  if(!is.null(results_n$est_short_term)){
    ...
    out$bias_short_term_Z0 <- mean(as.numeric(unlist(results_n$est_short_term_Z0)) - truth$short_term_Z0)
    out$bias_short_term_Z1 <- mean(as.numeric(unlist(results_n$est_short_term_Z1)) - truth$short_term_Z1)
  }
