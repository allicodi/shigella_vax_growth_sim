here::i_am("misc/provide_growth_effect_obs_abx.R")
...
data <- readRDS(here::here(paste0("misc/provide_data/shigella_provide_case_control.Rds")))
data$site <- "Bangladesh" # just need site not to break things

month <- 1

  Y_name <- paste0("haz_", month, "m")
  followup_age_name <- paste0("agemonth_", month, "m")
  
  data$followup_months <- data[[followup_age_name]] - data$agemonth_bl
  data$I_followup_months <- as.numeric(!is.na(data$followup_months))
  data$I_followup_months_x_followup_months <- ifelse(is.na(data$followup_months), 0, data$followup_months)
  
  if(msd){
    # Get IDs corresponding to MSD cases
    provide_data_msd_caseids <- unique(data$case_id[which(data$GEMS_MSD == 1)])
    
    # Subset to MSD cases and their matched controls
    data <- data[which(data$case_id %in% provide_data_msd_caseids),]
  } 
  
  if(lsd){
    provide_data_lsd_caseids <- unique(data$case_id[which(data$case == 1 & data$GEMS_MSD == 0)])
    
    # Subset to LSD cases and their matched controls
    data <- data[which(data$case_id %in% provide_data_lsd_caseids),]
  }
  
  if(ruuska_1){
    # Get IDs corresponding to MSD cases
    provide_data_msd_caseids <- unique(data$case_id[which(data$ruuska_sev == 1)])
  ...
  if(ruuska_0){
    provide_data_lsd_caseids <- unique(data$case_id[which(data$case == 1 & data$ruuska_sev == 0)])
  ...
  provide_case_data <- data[data$case == 1,]
  provide_control_data <- data[data$case == 0,]
  
  provide_case_data_imp <- impute_covariates(data = provide_case_data,
                                             site_var_name = "site", 
                                             imp_by_site = TRUE,
                                           imp_covariates = c("gender",
                                                              "agemonth_bl",
                                                              "agemonths_episode",
                                                              "bl_haz",
                                                              "medu_cat", 
  ...
  provide_control_data_imp <- impute_covariates(data = provide_control_data,
                                              imp_covariates = c("gender",
                                                                 "agemonth_bl",
                                                                 "agemonths_episode",
                                                                 "bl_haz",
  ...
  provide_case_data_imp <- provide_case_data_imp[,colnames(provide_case_data_imp) %in% colnames(data), drop = FALSE]
  provide_control_data_imp <- provide_control_data_imp[,colnames(provide_control_data_imp) %in% colnames(data), drop = FALSE]
  provide_data_imp <- rbind(provide_case_data_imp, provide_control_data_imp)
  
  # Add "age" and "enr_haz" to match wrappers (go back and rename in raw data later)
  provide_data_imp$age <- provide_data_imp$agemonth_bl
  provide_data_imp$enr_haz <- provide_data_imp$bl_haz
  
  one_hot_provide <- one_hot_encode(data = provide_data_imp,
                                  laz_var_name = Y_name,
                                  covariate_list = c("gender",
                                                     ...
                                                     "toil", 
                                                     "I_followup_months",
                                                     "I_followup_months_x_followup_months"),
                                  abx_var_name = "all_abx",
                                  site_var_name = "site",
                                  site_interaction = TRUE,
                                  severity_list = c("high_fever",
                                                    "dehyd_bin",
                                                    "lsstools",
                                                    "daysvomit",
                                                    "epi_duration"),
  ...
  # SET PARAMETERS
  data = one_hot_provide$data
  laz_var_name = Y_name
  abx_var_name = "all_abx"
  case_var_name = "case"
  followup_var_names = c("I_followup_months", "I_followup_months_x_followup_months")
  site_var_name = one_hot_provide$site_var_names
  covariate_list = one_hot_provide$covariate_list
  pathogen_quantity_list = c(      "adenovirus_40_41",                  
                                   "aeromonas",                         
                                   "astrovirus",                        
                                   "c_jejuni_coli",                 
                                   "cryptosporidium",                   
                                   ...
                                   "ETECLT",                              
                                   "EAEC")
  severity_list = one_hot_provide$severity_list
  first_id_var_name = "sid"
  ...
  msm_age_df <- data.frame(child_id = data$sid,
  ...
  results$aipw_models <- NULL
    ...
    if(msd){
      saveRDS(results, here::here(paste0("misc/results/provide/case_control/provide_case_control_no_abx_msd_", month, ".Rds")))
    } else if(lsd){
      saveRDS(results, here::here(paste0("misc/results/provide/case_control/provide_case_control_no_abx_lsd_", month, ".Rds")))
    } else if(ruuska_0){
      saveRDS(results, here::here(paste0("misc/results/provide/case_control/provide_case_control_no_abx_ruuska_lt_11_", month, ".Rds")))
    } else if(ruuska_1){
      saveRDS(results, here::here(paste0("misc/results/provide/case_control/provide_case_control_no_abx_ruuska_gt_11_", month, ".Rds")))
    }else{
      saveRDS(results, here::here(paste0("misc/results/provide/case_control/provide_case_control_no_abx_", month, ".Rds")))
    }
...
mo_list <- lapply(months, function(mo) {
  readRDS(here::here(sprintf("misc/results/provide/case_control/provide_case_control_no_abx_%d.Rds", mo)))
})

mo_list_severe <- lapply(months, function(mo) {
  readRDS(here::here(sprintf("misc/results/provide/case_control/provide_case_control_no_abx_msd_%d.Rds", mo)))
})

mo_list_mild <- lapply(months, function(mo) {
  readRDS(here::here(sprintf("misc/results/provide/case_control/provide_case_control_no_abx_lsd_%d.Rds", mo)))
})

# mo_list_severe <- lapply(months, function(mo) {
#   readRDS(here::here(sprintf("misc/results/provide/case_control/provide_case_control_no_abx_ruuska_gt_11_%d.Rds", mo)))
# })
# 
# mo_list_mild <- lapply(months, function(mo) {
#   readRDS(here::here(sprintf("misc/results/provide/case_control/provide_case_control_no_abx_ruuska_lt_11_%d.Rds", mo)))
# })
...
final_plot <- ggplot(combined_plot_data, aes(x = month_num, y = pt_est, color = group, fill = group)) +
  ...
  labs(
    x = "Month",
    y = "Growth Effect",
    title = "Longitudinal Shigella growth effects in PROVIDE with observed antibiotic use",
  ...
  theme_minimal(base_size = 14) +
  theme(legend.position = "bottom") + coord_cartesian(ylim = c(-0.6, 0.15))

ggsave(here::here("misc/results/provide/case_control/growth_effect_plot_gems_msd_zoom.jpeg"),final_plot,  dpi = 300, width = 12, height = 6)
